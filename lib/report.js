...
function truncate (str, width, right = false) {
  if (str.length <= width) return str
  return right ? '…' + str.slice(str.length - width + 1) : str.slice(0, width - 1) + '…'
}
...
function printHeader (nameWidth, uncoveredWidth) {
  ...
  const unc = pad(truncate('Uncovered Line #s', uncoveredWidth - 1), uncoveredWidth, true)
...
function printRow (fileSummary, fileName, nameWidth, uncoveredWidth) {
  ...
  console.log([
    ...
    pad(` ${truncate(uncoveredLines, uncoveredWidth - 2)} `, uncoveredWidth, true)
  ].join('|'))
}
...
function deriveWidths (groupedSummaries) {
  ...
  if (nameWidth + uncoveredWidth + STATIC_WIDTH > TERMINAL_WIDTH) {
    ...
    nameWidth = Math.floor(availableWidth / 2) - 1
    uncoveredWidth = availableWidth - nameWidth - 1
  }
