...
function isOutsideDir (dir, filename) {
  return process.platform === 'win32'
    ? !minimatch(path.resolve(dir, filename), path.join(dir, '**'), { dot: true, windowsPathsNoEscape: true })
    : /^\.\./.test(path.relative(dir, filename))
}

// Based off https://github.com/istanbuljs/test-exclude/blob/master/index.js with glob-related code removed
class TestExclude {
  constructor (opts = {}) {
