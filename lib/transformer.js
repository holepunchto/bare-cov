...
const { isAbsolute } = require(isBare ? 'bare-path' : 'path')
...
class Transformer {
  constructor (opts = {}) {
    ...
    this.exclude = new TestExclude({
      exclude: defaults.testExclude.exclude,
      include: [],
      extension: defaults.testExclude.extension,
      relativePath: true,
  ...
  async transformToCoverageMap (rawV8Report) {
    ...
    for (const v8ReportResult of v8Report.result) {
      ...
      for (const [path, coverage] of Object.entries(converted)) {
        if (coverages[path]) throw new Error(`Duplicate coverage for ${path}`)
