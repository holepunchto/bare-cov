...
const TestExclude = require('./exclude')

class Transformer {
  constructor (opts = {}) {
    this.includeRelative = opts.includeRelative ?? false
    this.exclude = new TestExclude({
      exclude: opts.exclude ?? defaultExclude,
      include: opts.include ?? [],
      extension: opts.extension ?? defaultExtension,
      relativePath: !(opts.allowExternal ?? false),
      excludeNodeModules: !(opts.includeNodeModules ?? false)
    })
  ...
  async transformToCoverageMap (v8Reports) {
    const processedReports = v8Reports.map((v8Report) => ({
